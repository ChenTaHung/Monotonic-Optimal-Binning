...
<p align = 'center'><img src = 'https://github.com/ChenTaHung/Monotonic-Optimal-Binning/blob/main/doc/charts/age-on-insuranceclai-PAVA-summary.png' alt = 'Image' style = 'width: 1200px'/></p>
...
In **[PAVA](https://github.com/ChenTaHung/Monotonic-Optimal-Binning/blob/main/doc/MOBPY-PAVA-PAVA.md)** module, we allow users to enter a dicitionary of additional variables comes with their statistics metric to show in the final PAVA result table. Unlike the MOB module can only calculate mean(average) for the metric, multiple statistics are acceptable in PAVA module, including `'count'`, `'mean'`, `'sum'`, `'std'`, `'var'`, `'min'`, `'max'`, `'ptp'`. It is noteworthy that the argument `add_var_aggFunc` usage is same as inputting a dictionary for different aggregations per column in **[pandas.DataFrame.agg](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.agg.html)**. See [PAVA documentation](https://github.com/ChenTaHung/Monotonic-Optimal-Binning/blob/main/doc/MOBPY-PAVA-PAVA.md) for more information.
...
- Testing Dataset : [US Health Insurance Dataset](https://www.kaggle.com/datasets/teertha/ushealthinsurancedataset) from [Kaggle](https://www.kaggle.com/)
